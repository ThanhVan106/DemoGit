...
// Xử lý AJAX lấy giá bán từ bảng gia_ban
if (isset($_GET['action']) && $_GET['action'] === 'get_price') {
  $product_id = (int)$_GET['product_id'];
  $order_date = isset($_GET['order_date']) ? $_GET['order_date'] : date('Y-m-d'); // lấy ngày từ GET hoặc dùng ngày hiện tại

  // Sửa truy vấn để lấy giá theo ngày bán chính xác
  $sql = "SELECT gia 
          FROM gia_ban 
          WHERE product_id = '{$product_id}' 
            AND ngay_ap_dung <= '{$order_date}' 
  ...
  if ($result) {
    echo json_encode(['gia' => $result['gia']]);
  } else {
    echo json_encode(['gia' => 0]);
  }
...
<!-- JavaScript hiển thị modal và xử lý sản phẩm -->
<script>
  ...
  function updatePrice(select) {
    ...
    const orderDate = document.querySelector('input[name="order_date"]').value; // lấy ngày bán
    ...
    fetch(`add_order.php?action=get_price&product_id=${productId}&order_date=${orderDate}`)
      .then(res => res.json())
      .then(data => {
        const price = parseFloat(data.gia || 0);
