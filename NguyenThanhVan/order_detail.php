...
// Lấy thông tin sản phẩm trong đơn hàng kèm đơn giá theo thời điểm đặt hàng
$order_items = find_by_sql("
  SELECT 
    oi.*, 
    p.name AS product_name,
    (
      SELECT gb.gia FROM gia_ban gb 
      WHERE gb.product_id = oi.product_id 
        AND gb.ngay_ap_dung <= '{$order_date}'
      ORDER BY gb.ngay_ap_dung DESC 
      LIMIT 1
    ) AS unit_price
  FROM order_items oi
        ...
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            ...
            <p><strong>Ngày đặt hàng:</strong> 
              <?php echo isset($order['order_date']) ? date('d/m/Y', strtotime($order['order_date'])) : 'Không xác định'; ?>
            </p>
