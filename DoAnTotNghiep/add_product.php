...
<?php
 if(isset($_POST['add_product'])){  // Nếu người dùng gửi biểu mẫu 'add_product'
   ...
   $req_fields = array('product-title','product-categorie','product-quantity','buying-price', 'saleing-price' );  // Các trường yêu cầu
   ...
   if(empty($errors)){  // Nếu không có lỗi
     ...
     $p_qty   = remove_junk($db->escape($_POST['product-quantity']));  // Loại bỏ ký tự đặc biệt từ số lượng sản phẩm
     $p_buy   = remove_junk($db->escape($_POST['buying-price']));  // Loại bỏ ký tự đặc biệt từ giá mua
     $p_sale  = remove_junk($db->escape($_POST['saleing-price']));  // Loại bỏ ký tự đặc biệt từ giá bán
     ...
     $query .=" name,quantity,buy_price,sale_price,categorie_id,media_id,date";  // Các cột trong bảng products
     $query .=") VALUES (";  // Bắt đầu các giá trị
     $query .=" '{$p_name}', '{$p_qty}', '{$p_buy}', '{$p_sale}', '{$p_cat}', '{$media_id}', '{$date}'";  // Các giá trị sẽ được chèn
     $query .=")";  // Kết thúc câu lệnh SQL
     $query .=" ON DUPLICATE KEY UPDATE name='{$p_name}'";  // Cập nhật nếu có khóa trùng lặp
              ...
              <div class="form-group">
               <div class="row">
                 ...
                 <div class="input-group">
                     ...
                     <input type="number" class="form-control" name="product-quantity" placeholder="Số lượng">  <!-- Ô nhập số lượng sản phẩm -->
                  </div>
                 </div>
                 <div class="col-md-4">
                   <div class="input-group">
                     ...
                     <input type="number" class="form-control" name="buying-price" placeholder="Giá mua">  <!-- Ô nhập giá mua -->
                  ...
                  <div class="col-md-4">
                    <div class="input-group">
                      ...
                      <input type="number" class="form-control" name="saleing-price" placeholder="Giá bán">  <!-- Ô nhập giá bán -->
                      <span class="input-group-addon">.00</span>
                   </div>
                  </div>
               </div>
