<?php
  ...
  $conn = mysqli_connect("localhost", "root", "", "sales_system");
  $success_message = '';
  $error_message = '';
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
      ...
      // Chèn thông tin vào bảng receipt_items
  ...
  $sql_products = "SELECT id, name, buy_price FROM products";
              ...
              <div class="form-group col-md-4">
                ...
                <select class="form-control product-select" name="product_id[]" required>
                  ...
                  <?php foreach ($products as $product): ?>
                    <option value="<?= $product['id']; ?>" data-buy-price="<?= $product['buy_price']; ?>"><?= $product['name']; ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
...
$(document).ready(function() {
  ...
  $('#product-details-container').on('change', '.product-select', function() {
    const selectedOption = $(this).find(':selected');
    const buyPrice = selectedOption.data('buy-price');
    const unitPriceInput = $(this).closest('.product-row').find('.unit-price-input');

    if (buyPrice !== undefined) {
      unitPriceInput.val(buyPrice);
